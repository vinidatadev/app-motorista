...
Sistema web de **gestão e atualização de cadastro de clientes** para equipes de motoristas, com **múltiplos endereços/contatos por cliente**, fluxo de **aprovação de endereços**, **carga em massa via Excel**, **fotos do local**, **geolocalização**, **navegação** (Waze / Google Maps / OpenStreetMap), **notificações em tempo real** (WebSocket) e **solicitações** abertas pelos usuários.

- **Frontend:** Vue 3 + Vite + Tailwind CSS + Leaflet (PWA).
- **Backend:** Python 3.12 + FastAPI + SQLAlchemy (async) + PostgreSQL + WebSocket.
...
2. **Pesquisa** — filtra clientes por estado/cidade/nome, visualiza todos os **endereços e contatos**, fotos, mapa e links de navegação. Quando o cliente **não é encontrado** ou os **contatos estão desatualizados**, o usuário pode abrir uma **solicitação**.
3. **Editar** — motorista edita o cadastro (várias lojas/endereços, cada um com seus contatos) e **submete para aprovação** (se não tiver permissão `aprovar`). Quem tem `aprovar`/admin edita direto.
4. **Aprovações** — aprovador revisa o diff (atual × proposto, incluindo endereços/contatos), aprova, edita antes de aprovar ou recusa. Aprovador e motorista recebem **notificações em tempo real**.
5. **Cadastrar** — cria novos clientes com **N endereços** e **N contatos por endereço**, com geocodificação automática.
6. **Carga Excel** — upload de `.xlsx` com preview (novos × alterados) e aplicação em lote (upsert por `codigo`). Formato: **uma linha por endereço**, com colunas de contatos.
7. **Solicitações** — perfil onde o time atende pedidos de cadastro/atualização abertos pelos usuários, com notificação de ida e volta.
8. **Notificações** — sino no header com badge, som e navegação ao clicar (via WebSocket, com refresh periódico de segurança).
9. **Admin de usuários** — cria/edita usuários, define empresa, role e permissões granulares.
...
│   ├── models.py            # User, Cliente, ClienteAlteracao, ClienteFoto, ClienteEndereco, ClienteContato, Notificacao, Solicitacao
...
│   ├── storage.py           # MinIO/S3: upload, delete, presign (URL pública), validação de imagem
│   ├── notify.py            # Gerenciador de conexões WebSocket + helpers de notificação
...
│   │   ├── notificacoes.py  # /api/notificacoes + WebSocket /ws/notificacoes
│   │   └── solicitacoes.py  # /api/solicitacoes
│   ├── requirements.txt
│   ├── Dockerfile
│   └── .env.example
├── src/                     # Frontend Vue 3
│   ├── main.js              # Bootstrap
│   ├── App.vue              # Layout (navbar com permissões, sino de notificações, footer)
...
│   │   └── useNotificacoes.js # WebSocket de notificações, som, lista reativa
│   └── views/
│       ├── LoginView.vue
│       ├── PesquisaView.vue
│       ├── EditarView.vue
...
| Cadastros | `full@app.com` | `full123` | visualizar, editar, criar, exportar, solicitacoes | SIN |
